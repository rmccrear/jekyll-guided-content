...
Use this prompt template to create lessons and levels using the jekyll-guided-content gem syntax.
...
- The subtitle appears in the utility bar navigation
- **Do NOT include numbers** - the JavaScript automatically numbers levels sequentially
- The `{% level %}` tag creates a level container

### Headers and Sections
- Main lesson title: `# [Title]` (in front matter and as H1)
- Level title inside level tag: `## [Level Title]` (should match the subtitle, but without numbers)
- Use `###` for major sections (What You'll Do, Instructions, etc.)
- Use `####` for subsections
- Use `---` (three dashes) as section separators within content
- **Important**: Do not include numbers in level titles - JavaScript handles numbering automatically
- **Important**: Do NOT use `##` for any other sections inside a level. All subsections must be `###` or smaller (H3, H4, etc.) to avoid breaking the TOC hierarchy.

### Utility Bar
- Always include `{% utility_bar %}` after the introduction and before the first level
- This creates the sticky navigation bar with level buttons and focus mode
- The utility bar automatically generates numbered buttons for each level (1, 2, 3, etc.)
...
8. **No Hard-Coded Numbers**: Do not include numbers in level titles - JavaScript handles this automatically
9. **Matching Subtitles**: Ensure the subtitle in `{% level %}` matches the H2 title (without numbers)
...
- Add `{% utility_bar %}` after the introduction
- Use `{% level subtitle="[Level Name]" %}` for each level (no numbers!)
- Use `## [Level Title]` for the level heading (no numbers!)
- JavaScript automatically numbers levels (1, 2, 3, etc.) in the utility bar
- Use `{% showme "Title" %}` for collapsible code hints
```
